...
/**
 * Sync Password Action
 */
if (!empty($_POST['syncpass_ipaddress'])) {
    $ip_address = $_POST['syncpass_ipaddress'];
    #$cyberarc_command = "cd /var/www/html/oneemstest/cyberark_test ; python update_accounts_bkup.py -i " . $ip_address;
    ...
    $cyberarc_output = "Sync Password Status : " . $cyberarc_output;
}

/**
 * Sync Password Action
 */
if (!empty($_POST['sshPuTTY_IPv6_addr'])) {
    $devicename = str_replace(':','-',$_POST['sshPuTTY_IPv6_addr']);
    $sshusername = $_SESSION['username'];
    $sshPuTTYline = "putty.exe -load \"PAMsession\" -ssh -l " . $sshusername . "@PAMadmin@" . $devicename . " -loghost PAMadmin@" . $devicename;
}
...
<br>
<input type="text" value='<?php echo $sshPuTTYline;?>' display = "readonly" size = "80" id="sshcmdoutput"> <button id="sshcmdoutputcpy"> Copy </button>
<?php //echo $sshPuTTYline; ?>
                                                                <!--  Sync Password Form End -->
