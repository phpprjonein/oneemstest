...
{
    global $APPCONFIG;
    $url = $APPCONFIG['api']['domain'].'/'.$endpoint; 
    $ch = curl_init($url);
...
function get_user_info($username, $password)
{
    ...
    $result = httpPost('login.php', $data_array);
...
function get_device_list_from_nodes_datatable($userid)
{
    $data_array =  array(
            "action"          => 'backup',
            "function"        => 'get_device_list_from_nodes_datatable',
            "userid"          => $userid
    );
    $result = httpPost('backup.php', $data_array);
    return json_decode($result, true);
}
...
function get_device_list_from_backuprestore_datatable($userid, $listname = '')
{
    ...
    $data_array =  array(
            "action"          => 'backup',
            "function"        => 'get_device_list_from_backuprestore_datatable',
            "userid"          => $userid,
            "listname"        => $listname
    );
    $result = httpPost('backup.php?'.http_build_query($_GET), $data_array);
    return json_decode($result, true);
}
...
function get_celltechusers_list($userid)
{
    $data_array =  array(
            "action"          => 'backup',
            "function"        => 'get_celltechusers_list',
            "userid"          => $userid
    );
    $result = httpPost('backup.php', $data_array);
    return json_decode($result, true);
...
{
    global $db2;
    $sql = "SELECT usrvarid, usrvarval FROM import_usrvars where usrvarname like 'interface-enodeb%'";
    $db2->query($sql);
    $resultset['result'] = $db2->resultset();
    return $resultset;
}
