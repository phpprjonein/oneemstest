$(document).ready(function() {
  $(document).on(
    "click",
    ".instant-health-check #sync-password-ip-health-check",
    function(event) {
      var req_err = false;
      $(".instant-health-check #status").html("");
      $(".instant-health-check #status").css("opacity", "");
      if (
        $(".instant-health-check #syncPassInputDeviceIPaddress").val() == ""
      ) {
        $(".instant-health-check #status").html(
          "<strong>Error!</strong> Sync Password field is required.<br/>"
        );
      ...
      $("#Modal_Health_Checks .modal-body").html(
        '<div class="text-center overlay box-body">Please wait... <div class="fa fa-refresh fa-spin" style="font-size:24px; text-align:center;"></div></div>'
      );
      myModal.modal("show");
      $.ajax({
        type: "post",
        ...
        data: {
          ctype: "Sync Password",
          syncpass_ipaddress: $("#syncPassInputDeviceIPaddress").val()
        },
  ...
  $(document).on(
    "click",
    ".instant-health-check #sync-cyberark-sync-password-ip-health-check",
    function(event) {
      var req_err = false;
      $(".instant-health-check #status").html("");
      $(".instant-health-check #status").css("opacity", "");
      if ($(".instant-health-check #synccyberarkpass_ipaddress").val() == "") {
        $(".instant-health-check #status").html(
          "<strong>Error!</strong> Check CyberArk Synchronization field is required.<br/>"
        );
        $(".instant-health-check #status").addClass("alert-danger");
        $(".instant-health-check #status").show();
        req_err = true;
      ...
      var myModal = $("#Modal_Health_Checks");
      $("#Modal_Health_Checks .modal-title").html(
        "Check CyberArk Synchronization Status"
      );
      $("#Modal_Health_Checks .modal-body").html(
        '<div class="text-center overlay box-body">Please wait... <div class="fa fa-refresh fa-spin" style="font-size:24px; text-align:center;"></div></div>'
      );
      ...
      $.ajax({
        ...
        data: {
          ctype: "SyncCyberArk",
          syncpass_ipaddress: $("#synccyberarkpass_ipaddress").val()
        },
        success: function(resdata) {
          $("#Modal_Health_Checks .modal-body").html(resdata);
          return false;
  ...
  $(document).on("click", ".instant-health-check #cyberark-add-acct", function(
    event
  ) {
    var req_err = false;
    $(".instant-health-check #status").html("");
    ...
    if ($(".instant-health-check #cyberarkaddacct_ipaddress").val() == "") {
      $(".instant-health-check #status").html(
        "<strong>Error!</strong> IPV6 address for adding Cyber Ark account is required.<br/>"
      );
      $(".instant-health-check #status").addClass("alert-danger");
      $(".instant-health-check #status").show();
      req_err = true;
    ...
    $.ajax({
      ...
      data: {
        ctype: "CyberArkadd",
        syncpass_ipaddress: $("#cyberarkaddacct_ipaddress").val()
      },
      success: function(resdata) {
        $("#Modal_Health_Checks .modal-body").html(resdata);
        return false;
  ...
  $(document).on(
    "click",
    ".instant-health-check #ssh-PuTTY-ip-health-check",
    function(event) {
      var req_err = false;
      $(".instant-health-check #status").html("");
      $(".instant-health-check #status").css("opacity", "");
      if ($(".instant-health-check #sshPuTTYInputIPv6").val() == "") {
        $(".instant-health-check #status").html(
          "<strong>Error!</strong> Show PuTTY CmdLine field is required.<br/>"
        );
        $(".instant-health-check #status").addClass("alert-danger");
        $(".instant-health-check #status").show();
        req_err = true;
      ...
      var sshPuTTYInputIPv6 = $(".instant-health-check #sshPuTTYInputIPv6")
        .val()
        .replace(/\:/g, "-");
      var command =
        $("#username").val() +
      ...
      //   Afotey add
      $("#Modal_Health_Checks .modal-body").append(
  ...
  $(document).on("click", "#ip-health-check", function(event) {
    ...
    if (
      $("#instant-health-check #inputDeviceIPaddress").val() == "" &&
      $("#instant-health-check #inputDeviceName").val() == ""
    ) {
      $("#instant-health-check #status").append(
        "<strong>Error!</strong> IP Address/Device Name field is required.<br/>"
      );
      $("#instant-health-check #status").addClass("alert-danger");
      req_err = true;
      if (req_err) {
    ...
    if ($("#instant-health-check #inputDeviceIPaddress").val() != "") {
      $.post("ip-mgt-process.php", {
        calltype: "trigger",
        action: "HC-IP-Validate-Disc",
        type: "IPv4ORIPv6",
        ipaddress: $("#instant-health-check #inputDeviceIPaddress").val()
      }).done(function(data) {
        if (data != "success") {
          $("#instant-health-check #status").append(
            "<strong>Error!</strong> Device IP Address is not valid.<br/>"
          );
          $("#instant-health-check #status").addClass("alert-danger");
          req_err = true;
          if (req_err) {
          ...
          var ajs = $.ajax({
            ...
            data: {
              deviceip: $("#inputDeviceIPaddress").val(),
              userid: $("#userid").val()
            },
            beforeSend: function() {
              $("#instant-health-check .panel-body-wrap").html(
