...
if (! empty($_GET['name'])) {
    ...
    if (!file_exists($name)) {
	 ...
	 jsonResponse(200, "Template Found", nl2br(file_get_contents($name)));
	//jsonResponse(200, "Template Found", readfile($name));
         /* $fileoutputcontent = "";
         while(!feof($file))  {
        $fileoutputcontent = $fileoutputcontent  + fgets($file);
      } 
	jsonResponse(200, "Template Found",$fileoutputcontent); 
      */
    }
} else {
