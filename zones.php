...
print '<pre>';
if(isset($resp_result_arr['user']['prefs']['calloutzones'])){
    foreach ($resp_result_arr['user']['prefs']['calloutzones'] as $key=>$val):
    $val = rawurlencode($val);
    $output_zones = @file_get_contents(('http://localhost/oneemstest/calloutzonesAPI/'.$_SESSION['username'].'_callout_zone_'.$val.'_list.php'));
